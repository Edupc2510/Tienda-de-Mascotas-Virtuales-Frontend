...
export function UsuariosProvider({ children }) {
  ...
  const tieneIdValido = (u) => u?.id !== null && u?.id !== undefined; // 👈 importante: permite id=0

  // ==========================
  // CARGAR USUARIOS Y ÓRDENES
  // - admin: trae TODAS las órdenes
  // - no admin: solo órdenes del usuario
  // ==========================
  useEffect(() => {
    const fetchInicial = async () => {
      try {
        ...
        // ✅ Si es admin, NO dependas del id (porque puede ser 0)
        const urlOrdenes = esAdmin(usuarioLogueado)
          ? `${API_URL}/ordenes`
          : tieneIdValido(usuarioLogueado)
  ...
  const cancelOrder = async (id) => {
    ...
    setOrdenes((prev) =>
      prev.map((o) => (o.id === id ? (data?.id ? data : { ...o, estado: "Cancelado" }) : o))
    );
  ...
  // ==========================
  // ACTIVAR/DESACTIVAR USUARIO (ADMIN) - local UI
  // ==========================
