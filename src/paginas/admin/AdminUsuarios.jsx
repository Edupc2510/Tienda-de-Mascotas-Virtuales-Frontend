...
export default function AdminUsuarios(){
  ...
  const filtered = usuarios.filter(u=> u.nombre.toLowerCase().startsWith(q.toLowerCase()) || String(u.id).startsWith(q) || (u.apellido||'').toLowerCase().startsWith(q.toLowerCase()) )
              ...
              <div style={{position:'relative',marginTop:12,gridColumn:'1/-1',width:'100%'}}>
                <div style={{marginTop:12,padding:12,borderTop:'1px solid #eee'}}>
                  ...
                  <div style={{marginTop:10,display:'flex',gap:8}}>
                    ...
                    <button onClick={()=> { setOpenId(null); setEditUser(null); }} style={{padding:'8px 12px',background:'#ccccccff',border:'1px solid #e5e7eb',borderRadius:6,cursor:'pointer'}}>Cancelar</button>
                  </div>
                </div>
              </div>
            )}
